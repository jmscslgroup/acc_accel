...
B_acc_accel_T acc_accel_B;

// Block states (default storage)
DW_acc_accel_T acc_accel_DW;

// Real-time model
RT_MODEL_acc_accel_T acc_accel_M_ = RT_MODEL_acc_accel_T();
RT_MODEL_acc_accel_T *const acc_accel_M = &acc_accel_M_;

// Model step function
void acc_accel_step(void)
{
  SL_Bus_acc_accel_std_msgs_Int32 rtb_BusAssignment1;
  SL_Bus_acc_accel_std_msgs_Int32 rtb_BusAssignment2;
  real_T rtb_Leader1;
  real_T value;
  real_T value_0;
  ...
  b_varargout_1 = Sub_acc_accel_648.getLatestMessage(&acc_accel_B.b_varargout_2);

  // Outputs for Enabled SubSystem: '<S8>/Enabled Subsystem' incorporates:
  //   EnablePort: '<S10>/Enable'

  if (b_varargout_1) {
    acc_accel_B.In1 = acc_accel_B.b_varargout_2;
  }
  ...
  b_varargout_1 = Sub_acc_accel_653.getLatestMessage(&acc_accel_B.b_varargout_2);

  // Outputs for Enabled SubSystem: '<S7>/Enabled Subsystem' incorporates:
  //   EnablePort: '<S9>/Enable'

  if (b_varargout_1) {
    acc_accel_B.In1_k = acc_accel_B.b_varargout_2;
  }
  ...
  // Sum: '<S1>/Leader1'
  rtb_Leader1 = acc_accel_B.In1_k.Linear.Z + acc_accel_B.In1.Linear.X;

  // MATLABSystem: '<S1>/Get Parameter'
  ParamGet_acc_accel_655.get_parameter(&value);

  // MATLABSystem: '<S1>/Get Parameter1'
  ParamGet_acc_accel_657.get_parameter(&value_0);
  ...
  if (value == -1.0) {
    if (rtb_Leader1 < 8.9408) {
      value = rtb_Leader1;
      if (fabs(acc_accel_B.In1.Linear.X - rtb_Leader1) < 2.0) {
        // BusAssignment: '<S1>/Bus Assignment2'
        rtb_BusAssignment2.Data = 2;
      } else if (acc_accel_B.In1.Linear.X > rtb_Leader1) {
        // BusAssignment: '<S1>/Bus Assignment2'
    ...
    } else if (acc_accel_B.In1.Linear.X > rtb_Leader1 + 2.0) {
      // BusAssignment: '<S1>/Bus Assignment2'
      rtb_BusAssignment2.Data = 1;
      value = rtb_Leader1;
    } else {
      // BusAssignment: '<S1>/Bus Assignment2'
      rtb_BusAssignment2.Data = 3;
      value = (acc_accel_B.In1.Linear.X + rtb_Leader1) / 2.0;
    }
  } else {
...
void acc_accel_initialize(void)
{
...

  {
    ...
    acc_accel_B.In1 = acc_accel_P.Out1_Y0_e;

    // End of SystemInitialize for SubSystem: '<S8>/Enabled Subsystem'

    // Start for MATLABSystem: '<S8>/SourceBlock'
    acc_accel_DW.obj_k.matlabCodegenIsDeleted = false;
    acc_accel_DW.obj_k.isInitialized = 1;
    b_zeroDelimTopic[0] = 'v';
    b_zeroDelimTopic[1] = 'e';
    b_zeroDelimTopic[2] = 'l';
    b_zeroDelimTopic[3] = '\x00';
    Sub_acc_accel_648.createSubscriber(&b_zeroDelimTopic[0], 1);
    acc_accel_DW.obj_k.isSetupComplete = true;
    ...
    acc_accel_B.In1_k = acc_accel_P.Out1_Y0;

    // End of SystemInitialize for SubSystem: '<S7>/Enabled Subsystem'

    // Start for MATLABSystem: '<S7>/SourceBlock'
    acc_accel_DW.obj_ks.matlabCodegenIsDeleted = false;
    acc_accel_DW.obj_ks.isInitialized = 1;
    for (i = 0; i < 7; i++) {
      b_zeroDelimTopic_0[i] = tmp[i];
    }

    b_zeroDelimTopic_0[7] = '\x00';
    Sub_acc_accel_653.createSubscriber(&b_zeroDelimTopic_0[0], 1);
    acc_accel_DW.obj_ks.isSetupComplete = true;
    ...
    acc_accel_DW.obj_l.matlabCodegenIsDeleted = false;
    acc_accel_DW.obj_l.isInitialized = 1;
    for (i = 0; i < 13; i++) {
      b_zeroDelimTopic_1[i] = tmp_0[i];
    }

    b_zeroDelimTopic_1[13] = '\x00';
    Pub_acc_accel_664.createPublisher(&b_zeroDelimTopic_1[0], 1);
    acc_accel_DW.obj_l.isSetupComplete = true;
    ...
    acc_accel_DW.obj_d.matlabCodegenIsDeleted = false;
    acc_accel_DW.obj_d.isInitialized = 1;
    for (i = 0; i < 11; i++) {
      b_zeroDelimTopic_2[i] = tmp_1[i];
    }

    b_zeroDelimTopic_2[11] = '\x00';
    Pub_acc_accel_661.createPublisher(&b_zeroDelimTopic_2[0], 1);
    acc_accel_DW.obj_d.isSetupComplete = true;

    // End of Start for MATLABSystem: '<S6>/SinkBlock'
    // End of SystemInitialize for SubSystem: '<S1>/Publish2'

    // Start for MATLABSystem: '<S1>/Get Parameter'
    acc_accel_DW.obj_a.matlabCodegenIsDeleted = false;
    acc_accel_DW.obj_a.isInitialized = 1;
    for (i = 0; i < 12; i++) {
      b_zeroDelimName[i] = tmp_2[i];
    }

    b_zeroDelimName[12] = '\x00';
    ParamGet_acc_accel_655.initialize(&b_zeroDelimName[0]);
    ParamGet_acc_accel_655.initialize_error_codes(0, 1, 2, 3);
    ParamGet_acc_accel_655.set_initial_value(25.0);
    acc_accel_DW.obj_a.isSetupComplete = true;

    // End of Start for MATLABSystem: '<S1>/Get Parameter'

    // Start for MATLABSystem: '<S1>/Get Parameter1'
    acc_accel_DW.obj.matlabCodegenIsDeleted = false;
    acc_accel_DW.obj.isInitialized = 1;
    for (i = 0; i < 11; i++) {
      b_zeroDelimTopic_2[i] = tmp_3[i];
    }

    b_zeroDelimTopic_2[11] = '\x00';
    ParamGet_acc_accel_657.initialize(&b_zeroDelimTopic_2[0]);
    ParamGet_acc_accel_657.initialize_error_codes(0, 1, 2, 3);
    ParamGet_acc_accel_657.set_initial_value(50.0);
    acc_accel_DW.obj.isSetupComplete = true;
...
void acc_accel_terminate(void)
{
  // Terminate for Atomic SubSystem: '<S1>/Subscribe8'
  // Terminate for MATLABSystem: '<S8>/SourceBlock'
  if (!acc_accel_DW.obj_k.matlabCodegenIsDeleted) {
    acc_accel_DW.obj_k.matlabCodegenIsDeleted = true;
  }
  ...
  if (!acc_accel_DW.obj_ks.matlabCodegenIsDeleted) {
    acc_accel_DW.obj_ks.matlabCodegenIsDeleted = true;
  }
  ...
  if (!acc_accel_DW.obj_a.matlabCodegenIsDeleted) {
    acc_accel_DW.obj_a.matlabCodegenIsDeleted = true;
  }

  // End of Terminate for MATLABSystem: '<S1>/Get Parameter'

  // Terminate for MATLABSystem: '<S1>/Get Parameter1'
  if (!acc_accel_DW.obj.matlabCodegenIsDeleted) {
    acc_accel_DW.obj.matlabCodegenIsDeleted = true;
  }
  ...
  if (!acc_accel_DW.obj_l.matlabCodegenIsDeleted) {
    acc_accel_DW.obj_l.matlabCodegenIsDeleted = true;
  }
  ...
  if (!acc_accel_DW.obj_d.matlabCodegenIsDeleted) {
    acc_accel_DW.obj_d.matlabCodegenIsDeleted = true;
  }
